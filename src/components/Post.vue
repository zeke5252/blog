<template>
    ...
    <div class="me-sm-0 me-md-5">
      <header class="mb-5 fixed-top">
        <font-awesome-icon
          icon="chevron-left"
          class="u-btn__prev"
          :style="!isPrevDisplay ? { opacity: 0.3 } : { opacity: 1 }"
          @click="doPrev"
        />
        ...
        <font-awesome-icon
          icon="chevron-right"
          class="u-btn__next"
          :style="!isNextDisplay ? { opacity: 0.3 } : { opacity: 1 }"
          @click="doNext"
        />
      </header>
      <section class="my-3">
  ...
  <div class="row">
    <div class="col-sm-6 mb-6">
      <button
        type="button"
        class="col-12 col-md-6 offset-md-6 mb-5 mt-1"
        style="height: 40px"
...
export default {
  ...
  setup(props) {
    ...
    const doPrev = () => {
      let toTitle = store.getters.GET_NEXT_OR_PREV_POST(
        route.params.title,
        false
      );
      router.push(toTitle);
