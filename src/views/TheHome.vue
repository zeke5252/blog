<template>
	...
	<div v-else class="home">
		<div class="d-flex justify-content-end fixed-top pe-3 pe-md-4 mt-1">
			...
			<input class="search--input p-1" v-model="keyword" type="text" list="keywordListOptions" id="searchInput"
		...
		<div class="row row-cols-1 row-cols-md-3 g-4 pe-sm-0 pe-md-2">
			<div class="position-relative" v-for="post in keywordPosts.length > 0 ? keywordPosts : postsCollector"
				:key="post.title">
				<router-link :to="`/posts/${post.title}`" style="color: white"
					:class="post.imageFiles.length > 0 ? 'card text-decoration-none styleImg' : 'card text-decoration-none styleImg styleTxt'">
					<div v-if="post.imageFiles.length > 0" style="overflow: hidden">
						<BasePhoto :Url="PhotoAPI.getSrc(post.imageFiles, true)" :Images="post.imageFiles" :showExif="false"
							:showBorder="false" />
					</div>
				...
				<button v-if="isLogin" class="btn btn-primary rounded-circle translate-middle position-absolute top-0"
					@click="removePost(post)">
					<font-awesome-icon icon="trash" />
				</button>
				<div class="position-absolute bottom-0 end-0 px-3 m-2 fw-lighter text-primary border-start border-1 border-light"
					style="letter-spacing: 4px">{{ getConvertTime(post.created) }}</div>
			</div>
		</div>
...
export default defineComponent({
	...
	setup() {
		...
		const loadMore = () => {
			...
			const isScrollNearBottom = scrollTop + windowHeight >= (scrollHeight * 4) / 5;
		...
		const getFirstParagraph = (content) => {
			...
			if (!contentsArr || !Array.isArray(contentsArr)) return contentsArr || "No contents!";
			const result = contentsArr.find(el => el.substring(0, 4) !== "http");
			return result ? ContentAPI.limitStrSize(ContentAPI.removeMarks(result), 150) : "No contents!";
		};
		...
		const removeDBImages = (post) => {
			...
			const imagesToDelete = imagesUrls.map(url => url.match(/(?<=%2F).*(?=,|$)/)[0]);
